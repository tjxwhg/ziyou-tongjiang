// js/trip-planner.js - 智能行程规划引擎（完整稳定版）
...
export class TripPlanner {
    ...
    async plan() {
        ...
        while (queue.length > 0 && this.currentDay <= this.maxDays && processCount < MAX_ATTEMPTS) {
            ...
            if (travel > 180) {
                this.warnings.push(`❌ 从 ${this.getPoiById(this.lastPoiId).name} 到 ${poi.name} 的交通耗时 ${travel} 分钟（超过 180 分钟限制），请检查数据。`);
                continue; // 跳过该景点
            }
